    ...
    <main className="bg-grid min-h-dvh">
      <div className="mx-auto max-w-5xl px-6 py-10">
        <header className="mb-8 flex items-start justify-between gap-4">
          <div>
            <h1 className="flex items-center gap-2.5 font-mono text-[19px] font-bold tracking-tight text-ink">
              <span className="inline-block size-2 rounded-full bg-accent" />
              Sentinel
            </h1>
            <p className="mt-1.5 max-w-xl text-[12px] leading-relaxed text-ink-dim">
              Agentic AI red-team auditor. Adversarially probes a target agent for
              jailbreaks, guardrail bypasses, data leaks and unsafe tool calls,
              then produces a severity-scored, reproducible report.
            </p>
          </div>
          <div className="flex shrink-0 flex-col items-end gap-1.5">
            ...
            {health ? (
              <span className="font-mono text-[10px] text-ink-mute">
                {health.profile} · cap {compactUsd(health.budget_cap)}
              </span>
            ) : null}
          </div>
        </header>
