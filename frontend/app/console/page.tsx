...
import { prettyCategory, truncate } from "@/lib/format";
 ...
 *
 * No `theme-light` wrapper, unlike where this used to live. The console and the
 * run views are dark by choice — it is a tool that sits next to a terminal, and
 * globals.css says so at the top — and it only ran light before because it was
 * nested inside the landing page's light subtree along with the narrative bands.
 * Standing on its own it matches /runs/[runId] instead.
 *
 * No SmoothScroll either: Lenis is mounted on the landing page deliberately, and
 * this page is a form above a list rather than something you glide through.
 */
      ...
      <div className="mx-auto max-w-5xl px-6 py-16">
        <header className="mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 font-display text-[clamp(1.7rem,3.4vw,2.6rem)] leading-[1.15] text-ink">
              Authorize a scope
            </h1>
          ...
          {/* The way back, now that this is a route rather than a section the
              landing page scrolls to. */}
          <Link
            href="/"
            className="shrink-0 font-mono text-[11px] text-ink-mute transition hover:text-ink-dim"
          >
            ← Sentinel
          </Link>
        </header>
